...
export async function updateDonorProfile(
  ...
  // Profile fields -> public.donors (visible to other signed-in donors)
  const dbUpdates: any = {};
  ...
  // TTI / health fields -> public.donor_health (owner-only, never shared)
  ...
  if (Object.keys(dbUpdates).length === 0) {
    // Only health changed — reload the profile so the caller gets fresh data.
...
// ============ SUPABASE: own health record ============
// Only ever returns the signed-in donor's own row — RLS on donor_health makes
// any attempt to read someone else's record come back empty.
...
export async function fetchSharedData(
  ...
  // Neither source exposes health/TTI data. Those live in donor_health and are
  // readable only by the donor themselves — see fetchMyHealthInfo().
